...
from gps_spoofing_detection import detect_location_anomalies, detect_speed_course_inconsistencies, detect_neighbour_conflicts
...
def process_chunk(
    ...
    # Remove rows with missing/invalid lat/lon and retain rows with valid lat/lon
    ...
    for _, vessel_df in mmsi_groups:
        updated_vessel_df, vessel_anolmaly_df = detect_location_anomalies(vessel_df,timestamp_col,location_speed_threshold)
        if not vessel_anolmaly_df.empty:
            location_anomalies_list.append(vessel_anolmaly_df)
        
        updated_vessel_df2, speed_course_anom_df = detect_speed_course_inconsistencies(updated_vessel_df, timestamp_col, speed_diff_threshold, bearing_diff_threshold)
    ...
    # cross_vessel_anomalies_df = detect_neighbour_conflicts(
    #     cleaned_chunk,
    #     timestamp_col,
    #     time_window_minutes,
