...
def detect_location_anomalies(
    ...
    vessel_df["dist_km"] = haversine_distance(
...
def compute_bearing(lat1, lon1, lat2, lon2):
    ...
    d_lon_rad = np.radians(lon2 - lon1)

    x = np.sin(d_lon_rad) * np.cos(lat2_rad)
    y = (np.cos(lat1_rad) * np.sin(lat2_rad) - np.sin(lat1_rad) * np.cos(lat2_rad) * np.cos(d_lon_rad))
...
def detect_speed_course_inconsistencies(
    vessel_df: pd.DataFrame,
    timestamp_col: str = "# Timestamp",
    ...
    # If SOG or COG is missing, set them to 0
    ...
    df["computed_bearing"] = compute_bearing(df["lat_shift2"], df["lon_shift2"], df["Latitude"], df["Longitude"])
...
def detect_neighbour_conflicts(
